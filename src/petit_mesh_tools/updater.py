...
class GitSettings:
    owner = "samia-done"
    repository = "Petit-Mesh-Tools"
    branches = ["main"]
    default_target_addon_path = "petit_mesh_tools"
    target_addon_path = {
        "main": "src{}petit_mesh_tools".format(get_separator()),
    }
    update_text = 'Check add-on update'


@BlClassRegistry()
class PMT_OT_CheckAddonUpdate(bpy.types.Operator):
    bl_idname = "mesh.pmt_check_addon_update"
...
@BlClassRegistry()
class PMT_OT_UpdateAddon(bpy.types.Operator):
    bl_idname = "mesh.pmt_update_addon"
    bl_label = "Update"
...
def draw_updater_ui(prefs_obj):
    ...
    if not updater.candidate_checked():
        ...
        col.scale_y = 2
        row = col.row()
        row.operator(PMT_OT_CheckAddonUpdate.bl_idname,
        ...
        row.scale_y = 2
        col = row.column()
        col.operator(PMT_OT_CheckAddonUpdate.bl_idname,
                     text=GitSettings.update_text,
                     icon='FILE_REFRESH')
        col = row.column()
        ...
        ops = row.operator(
            PMT_OT_UpdateAddon.bl_idname, text="Update Now",
            icon='TRIA_DOWN_BAR')
